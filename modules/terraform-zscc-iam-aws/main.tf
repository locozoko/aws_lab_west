...
resource "aws_iam_role" "cc_node_iam_role" {
  count              = var.byo_iam == false ? var.iam_count : 0
  name               = var.iam_count > 1 ? "${var.name_prefix}-cc-${count.index + 1}-node-iam-role" : "${var.name_prefix}-cc_node_iam_role"
...
resource "aws_iam_instance_profile" "cc_host_profile" {
  count = var.byo_iam == false ? var.iam_count : 0
  name  = var.iam_count > 1 ? "${var.name_prefix}-cc-${count.index + 1}-host-profile" : "${var.name_prefix}-cc-host-profile"
  role  = aws_iam_role.cc_node_iam_role[count.index].name
