...
resource "aws_security_group" "cc_mgmt_sg" {
  ...
  name        = var.sg_count > 1 ? "${var.name_prefix}-cc-${count.index + 1}-mgmt-sg" : "${var.name_prefix}-cc-mgmt-sg"
...
resource "aws_security_group" "cc_service_sg" {
  count       = var.byo_security_group == false ? var.sg_count : 0
  name        = var.sg_count > 1 ? "${var.name_prefix}-cc-${count.index + 1}-svc-sg" : "${var.name_prefix}-cc-svc-sg"
